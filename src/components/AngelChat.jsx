...
const AngelChat = ({ comment, handleClick }) => {
    const { text, isEnd } = useTyping(comment);

    return (
        <div>
            <div className=" p-8 left-72 bottom-10 absolute z-50 bg-white w-[1000px] h-[300px] rounded-[44px] border-4 border-login-pink">
                <p className=" font-medium font-SB text-xl">
                    {text}
                    <span className={`${isEnd ? 'hidden' : 'animate-typing'} `}>|</span>
                </p>
            </div>
            <div className="flex items-center justify-center z-50 absolute left-[360px] bottom-[316px] w-52 h-12 rounded-3xl" style={{ backgroundColor: "#F6609A" }}>
